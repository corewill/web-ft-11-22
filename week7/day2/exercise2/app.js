...
app.get("/names",(request,response) =>{
 ...
 let names = []
    for(let person of people){
        let newNames = person.name
        names.push(newNames)
    }
response.send(names)
})

app.get("/phoneNumber",(request,response) =>{
    let phoneNumber = []
    for(let person of people){
        if(person.age === 24){
            phoneNumber.push(person.phone)
        }
    }
response.send(phoneNumber)
    })

app.get("/over24under26",(request,response) =>{
    let selected = []
    for(let person of people){
...
app.get("/emails",(request,response) =>{
        let emails = []
           for(let person of people){
               let newEmails = person.email
               emails.push(newEmails)
           }
       response.send(emails)
       })

app.get("/combinedBalance"),(request,response) =>{
    let combinedBalance = []
    for(let person of people){
        let newBalance = int.person.balance
        combinedBalance.push(newBalance)
    }
response.send(combinedBalance)
}
